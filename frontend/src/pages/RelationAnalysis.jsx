...
function scrollToRelationSection(event, sectionId) {
...
export default function RelationAnalysis() {
  ...
  return (
    <section className="page relation-analysis-page">
      ...
      {data && <>
        <nav className="relation-section-nav" aria-label="Relation Analysis sections">
          <a href="#relation-overview" onClick={(event) => scrollToRelationSection(event, 'relation-overview')}>Overview</a>
          <a href="#relation-paired" onClick={(event) => scrollToRelationSection(event, 'relation-paired')}>Paired effects</a>
          <a href="#relation-details" onClick={(event) => scrollToRelationSection(event, 'relation-details')}>Detailed results</a>
          <a href="#relation-historical" onClick={(event) => scrollToRelationSection(event, 'relation-historical')}>Historical</a>
        </nav>

        <section id="relation-overview" className="relation-major-section">
          <div className="relation-section-heading">
            <span className="eyebrow">Key takeaway</span>
            <h2>Largest descriptive mean improvement</h2>
            <p>All pointwise 95% paired intervals include zero.</p>
          </div>
          <article className="summary-card accent-card relation-takeaway-card">
            <div>
              <span className="card-kicker">Largest descriptive mean improvement</span>
              <strong>{data.results[0].relation}</strong>
            </div>
            <div className="relation-takeaway-metrics">
              <div><strong>{signed(data.results[0].delta_mrr_mean)}</strong><span>mean ΔMRR</span></div>
              <div><strong>{data.results[0].wins_vs_g0}/5</strong><span>wins vs G0</span></div>
            </div>
          </article>

          <div className="relation-subsection-heading">
            <span className="eyebrow">Study scope / coverage</span>
            <h3>Verified five-seed scope</h3>
          </div>
          <div className="analysis-summary-grid relation-scope-grid">
            <article className="summary-card">
              <span>Verified study coverage</span>
              <strong>{data.relation_runs} relation runs</strong>
              <p>7 relations × 5 seeds, paired with {data.paired_baseline_runs} G0 runs</p>
            </article>
            <article className="summary-card">
              <span>Five-seed G0 baseline</span>
        ...
        <section id="relation-paired" className="relation-major-section">
          <div className="relation-section-heading">
        ...
        <section id="relation-historical" className="relation-major-section relation-historical-section">
          <div className="relation-section-heading">
            <span className="eyebrow">Historical / earlier exploratory analysis</span>
            <h2>{data.history.label}</h2>
            <p>This earlier three-seed ranking result is preserved as history. Its ordering changed after adding seeds 45 and 46; it is not the current study estimate.</p>
          </div>
          <div className="section-block relation-historical-card">
            <div className="results-table-wrap"><table className="results-table"><thead><tr><th>Relation</th><th>Mean MRR</th><th>Mean ΔMRR</th><th>Wins</th></tr></thead><tbody>{data.history.results.map((row) => <tr key={row.graph}><td>{row.relation}</td><td>{row.mrr_mean.toFixed(6)}</td><td>{signed(row.delta_mrr_mean)}</td><td>{row.wins_vs_g0}/3</td></tr>)}</tbody></table></div>
          </div>
        </section>

        <section className="relation-major-section relation-limitations-section">
          <div className="relation-section-heading">
